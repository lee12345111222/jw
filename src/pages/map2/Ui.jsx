...
import { Search, Detail, Zoom, Select, PassCards } from './components/index';
...
const createPopup = (x, y, name = '', area = '', time = 5, isShip = false) => {
  return `
<div class='popup-title' onclick="cp('${name}')">${name}</div>
  ...
  useEffect(() => {
    ...
    setHouses(() =>
      Object.values(mapData).map(
        ({ x1, y1, x2, y2, token_id, owner, custom_name, custom_img }) => ({
          startX: x1,
          startY: y1,
          endX: x2,
          endY: y2,
      ...
      const group = L.layerGroup(
        houses
          ...
          .map(({ startX, startY, endX, endY, custom_img }) => {
            return L.imageOverlay(custom_img || `/map_static/sold0.svg`, [
              [startX + 1, startY + 1],
              [endX - 1, endY - 1]
            ]);
          })
        ...
        houses
          ...
          .map(({ startX, startY, endX, endY, status, custom_img }) => {
            return L.imageOverlay(
              custom_img || `/map_static/sold${status}.svg`,
              [
                [startX + 1, startY + 1],
                [endX - 1, endY - 1]
              ]
