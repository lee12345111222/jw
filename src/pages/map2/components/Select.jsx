...
import { getsysdefaults } from '@/api/map';
...
const Select = ({
  ...
  const { data: sysdefaults, run } = useApi(getsysdefaults, { manual: true });

  const sysDefault = useMemo(
    () =>
      houses
        .filter(({ tokenId }) =>
          sysdefaults?.data?.result?.find((item) => +item === +tokenId)
        )
        .map(({ startX, startY, endX }) =>
          markCreator(startX, startY, endX - startX)
        ),
    [houses, sysdefaults]
  );

  useEffect(() => {
    run({ login_token: token, address: account });
  }, [account, run, token]);
  ...
  const handleRagion = useCallback(
    ...
    [areaLayer, center, defaultCenter, height, map, width]
  );
